...
unsafe extern "C" fn execSql(
    ...
    loop {
        ...
        let mut zSubSql: *const ::core::ffi::c_char =
            sqlite3_column_text(pStmt, 0 as ::core::ffi::c_int) as *const ::core::ffi::c_char;
        if !(!zSubSql.is_null()
            && (strncmp(
                zSubSql,
...
unsafe extern "C" fn execSqlF(
    ...
    let mut rc: ::core::ffi::c_int = 0;
    ap = args.clone();
    z = sqlite3VMPrintf(db, zSql, ap.as_va_list());
    if z.is_null() {
        return SQLITE_NOMEM;
    }
...
pub unsafe extern "C" fn sqlite3RunVacuum(
    ...
    if !pOut.is_null() {
        ...
        (*db).openFlags |= (SQLITE_OPEN_CREATE | SQLITE_OPEN_READWRITE) as ::core::ffi::c_uint;
    } else {
    ...
    (*db).flags |= (SQLITE_WriteSchema | SQLITE_IgnoreChecks) as u64_0
        | SQLITE_Comments
        | SQLITE_AttachCreate
        | SQLITE_AttachWrite;
    (*db).mDbFlags |= (DBFLAG_PreferBuiltin | DBFLAG_Vacuum) as u32_0;
    (*db).flags &= !((SQLITE_ForeignKeys | SQLITE_ReverseOrder | SQLITE_Defensive) as u64_0
        | SQLITE_CountRows);
    (*db).mTrace = 0 as u8_0;
    ...
    if !(rc != SQLITE_OK) {
        ...
        pTemp = (*pDb).pBt;
        if !pOut.is_null() {
            let mut id: *mut sqlite3_file =
                sqlite3PagerFile(sqlite3BtreePager(pTemp) as *mut Pager);
            let mut sz: i64_0 = 0 as i64_0;
            if !(*id).pMethods.is_null()
                && (sqlite3OsFileSize(id, &raw mut sz) != SQLITE_OK || sz > 0 as i64_0)
            {
                rc = SQLITE_ERROR;
                sqlite3SetString(
                    pzErrMsg,
                ...
                (*db).mDbFlags |= DBFLAG_VacuumInto as u32_0;
                pgflags = ((*(*db).aDb.offset(iDb as isize)).safety_level as u64_0
                    | (*db).flags & PAGER_FLAGS_MASK as u64_0) as u32_0;
        ...
        match current_block {
            ...
            _ => {
                ...
                sqlite3BtreeSetPagerFlags(
                    pTemp,
                    pgflags as ::core::ffi::c_uint | PAGER_CACHESPILL as ::core::ffi::c_uint,
                );
                rc = execSql(
                    db,
                    pzErrMsg,
                ...
                if !(rc != SQLITE_OK) {
                    ...
                    if !(rc != SQLITE_OK) {
                        if sqlite3PagerGetJournalMode(sqlite3BtreePager(pMain) as *mut Pager)
                            ...
                            || (*db).mallocFailed as ::core::ffi::c_int != 0
                        {
                            ...
                            if !(rc != SQLITE_OK) {
                                ...
                                if !(rc != SQLITE_OK) {
                                    ...
                                    (*db).mDbFlags &= !DBFLAG_Vacuum as u32_0;
                                    if !(rc != SQLITE_OK) {
                                        rc = execSqlF(
                                            db,
                                            pzErrMsg,

...
pub unsafe extern "C" fn sqlite3RowSetInit(mut db: *mut sqlite3) -> *mut RowSet {
    let mut p: *mut RowSet =
        sqlite3DbMallocRawNN(db, ::core::mem::size_of::<RowSet>() as u64_0) as *mut RowSet;
    if !p.is_null() {
        let mut N: ::core::ffi::c_int = sqlite3DbMallocSize(db, p as *const ::core::ffi::c_void);
...
unsafe extern "C" fn rowSetEntryAlloc(mut p: *mut RowSet) -> *mut RowSetEntry {
    if (*p).nFresh as ::core::ffi::c_int == 0 as ::core::ffi::c_int {
        ...
        pNew = sqlite3DbMallocRawNN((*p).db, ::core::mem::size_of::<RowSetChunk>() as u64_0)
            as *mut RowSetChunk;
        if pNew.is_null() {
...
pub unsafe extern "C" fn sqlite3RowSetInsert(mut p: *mut RowSet, mut rowid: i64_0) {
    ...
    if !pLast.is_null() {
        if rowid <= (*pLast).v {
            (*p).rsFlags = ((*p).rsFlags as ::core::ffi::c_int & !ROWSET_SORTED) as u16_0;
        }
        (*pLast).pRight = pEntry;
    } else {
        (*p).pEntry = pEntry;
...
pub unsafe extern "C" fn sqlite3RowSetNext(
    ...
    if (*p).rsFlags as ::core::ffi::c_int & ROWSET_NEXT == 0 as ::core::ffi::c_int {
        ...
        (*p).rsFlags =
            ((*p).rsFlags as ::core::ffi::c_int | (ROWSET_SORTED | ROWSET_NEXT)) as u16_0;
    }
    if !(*p).pEntry.is_null() {
        *pRowid = (*(*p).pEntry).v;
...
pub unsafe extern "C" fn sqlite3RowSetTest(
    ...
    if iBatch != (*pRowSet).iBatch {
        ...
        if !p.is_null() {
            ...
            (*pRowSet).rsFlags =
                ((*pRowSet).rsFlags as ::core::ffi::c_int | ROWSET_SORTED) as u16_0;
        }

...
pub unsafe extern "C" fn sqlite3AuthReadCol(
    ...
    rc = (*db).xAuth.expect("non-null function pointer")(
        (*db).pAuthArg,
        SQLITE_READ,
        zTab,
        zCol,
...
pub unsafe extern "C" fn sqlite3AuthRead(
    ...
    if (*pExpr).op as ::core::ffi::c_int == TK_TRIGGER {
        ...
        while iSrc < (*pTabList).nSrc {
            if (*pExpr).iTable
                == (*(&raw mut (*pTabList).a as *mut SrcItem).offset(iSrc as isize)).iCursor
            {
                pTab = (*(&raw mut (*pTabList).a as *mut SrcItem).offset(iSrc as isize)).pSTab;
                break;
            } else {
                iSrc += 1;
            }

...
unsafe extern "C" fn backupOnePage(
    ...
    let nSrcPgsz: ::core::ffi::c_int = sqlite3BtreeGetPageSize((*p).pSrc) as ::core::ffi::c_int;
    ...
    while rc == SQLITE_OK && iOff < iEnd {
        ...
        let mut iDest: Pgno = ((iOff / nDestPgsz as i64_0) as Pgno).wrapping_add(1 as Pgno);
        if !(iDest
            == (sqlite3PendingByte as u32_0)
                .wrapping_div((*(*(*p).pDest).pBt).pageSize)
                .wrapping_add(1 as u32_0))
        {
            rc = sqlite3PagerGet(pDestPager, iDest, &raw mut pDestPg, 0 as ::core::ffi::c_int);
            if SQLITE_OK == rc && {
                rc = sqlite3PagerWrite(pDestPg);
                SQLITE_OK == rc
            } {
                let mut zIn: *const u8_0 =
                    zSrcData.offset((iOff % nSrcPgsz as i64_0) as isize) as *const u8_0;
                let mut zDestData: *mut u8_0 = sqlite3PagerGetData(pDestPg) as *mut u8_0;
                let mut zOut: *mut u8_0 =
                    zDestData.offset((iOff % nDestPgsz as i64_0) as isize) as *mut u8_0;
                memcpy(
                ...
                *(sqlite3PagerGetExtra(pDestPg) as *mut u8_0)
                ...
                if iOff == 0 as i64_0 && bUpdate == 0 as ::core::ffi::c_int {
                    sqlite3Put4byte(
...
pub unsafe extern "C" fn sqlite3_backup_step(
    ...
    if isFatalError(rc) == 0 {
        ...
        destMode = sqlite3PagerGetJournalMode(sqlite3BtreePager((*p).pDest) as *mut Pager);
        if SQLITE_OK == rc
            && (destMode == PAGER_JOURNALMODE_WAL || sqlite3PagerIsMemdb(pDestPager) != 0)
            && pgszSrc != pgszDest
        {
            rc = SQLITE_READONLY;
        }
            ...
            && rc == 0
        {
                    ...
                    .wrapping_add(1 as u32_0)
            {
                ...
                rc = sqlite3PagerGet(pSrcPager, iSrcPg, &raw mut pSrcPg, PAGER_GET_READONLY);
                if rc == SQLITE_OK {
                    rc = backupOnePage(
                        p,
                        iSrcPg,
        ...
        if rc == SQLITE_OK {
            ...
            (*p).nRemaining =
                ((nSrcPage + 1 as ::core::ffi::c_int) as Pgno).wrapping_sub((*p).iNext);
        ...
        if rc == SQLITE_DONE {
            ...
            if rc == SQLITE_OK {
                ...
                if pgszSrc < pgszDest {
                    ...
                    let pFile: *mut sqlite3_file =
                        sqlite3PagerFile(pDestPager) as *mut sqlite3_file;
                    let mut iPg: Pgno = 0;
                    ...
                    while rc == SQLITE_OK && iPg <= nDstPage as Pgno {
                        if iPg
                            != (sqlite3PendingByte as u32_0)
                                ...
                                .wrapping_add(1 as u32_0)
                        {
                            let mut pPg: *mut DbPage = ::core::ptr::null_mut::<DbPage>();
                            rc = sqlite3PagerGet(
                                pDestPager,
                                iPg,
                                &raw mut pPg,
                    ...
                    iEnd = if ((sqlite3PendingByte + pgszDest) as i64_0) < iSize {
                        (sqlite3PendingByte + pgszDest) as i64_0
                    } else {
                        iSize
                    };
                    iOff = (sqlite3PendingByte + pgszSrc) as i64_0;
                    while rc == SQLITE_OK && iOff < iEnd {
                        let mut pSrcPg_0: *mut PgHdr = ::core::ptr::null_mut::<PgHdr>();
                        let iSrcPg_0: Pgno = (iOff / pgszSrc as i64_0 + 1 as i64_0) as Pgno;
                        rc = sqlite3PagerGet(
                            pSrcPager,
                            iSrcPg_0,
                            &raw mut pSrcPg_0,
                        ...
                        if rc == SQLITE_OK {
                            let mut zData: *mut u8_0 =
                                sqlite3PagerGetData(pSrcPg_0 as *mut DbPage) as *mut u8_0;
                            rc = sqlite3OsWrite(
                                pFile,
                ...
                if SQLITE_OK == rc && {
                    rc = sqlite3BtreeCommitPhaseTwo((*p).pDest, 0 as ::core::ffi::c_int);
                    SQLITE_OK == rc
                } {
                    rc = SQLITE_DONE;
                }
        ...
        if bCloseTrans != 0 {
            sqlite3BtreeCommitPhaseOne((*p).pSrc, ::core::ptr::null::<::core::ffi::c_char>());
...
pub unsafe extern "C" fn sqlite3BtreeCopyFile(
    ...
    if !(*pFd).pMethods.is_null() {
        let mut nByte: i64_0 =
            sqlite3BtreeGetPageSize(pFrom) as i64_0 * sqlite3BtreeLastPage(pFrom) as i64_0;
    ...
    match current_block {
        17216689946888361452 => {
            ...
            if rc == SQLITE_OK {
                (*(*pTo).pBt).btsFlags =
                    ((*(*pTo).pBt).btsFlags as ::core::ffi::c_int & !BTS_PAGESIZE_FIXED) as u16_0;
            } else {

...
unsafe extern "C" fn rtreeNodeOfFirstSearchPoint(
    ...
    if (*pCur).aNode[ii as usize].is_null() {
        id = if ii != 0 {
            (*(*pCur).aPoint.offset(0 as isize)).id
        } else {
            (*pCur).sPoint.id
        };
        *pRC = nodeAcquire(
...
unsafe extern "C" fn rtreeSearchPointPop(mut p: *mut RtreeCursor) {
    ...
    if (*p).bPoint != 0 {
        ...
        (*p).anQueue[(*(*p).aPoint.offset(0 as isize)).iLevel as usize] =
            (*p).anQueue[(*(*p).aPoint.offset(0 as isize)).iLevel as usize]
                .wrapping_sub(1);
        (*p).nPoint -= 1;
        n = (*p).nPoint;
        *(*p).aPoint.offset(0 as isize) = *(*p).aPoint.offset(n as isize);
...
unsafe extern "C" fn geomCallback(
    ...
    if pBlob.is_null() {
        ...
        (*pBlob).cb = *pGeomCtx.offset(0 as isize);
